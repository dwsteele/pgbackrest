...
- Also for implementation: archive-get caches a single archive cipher pass per repo (`ArchiveGetFindCacheRepo.cipherPassArchive`, `src/command/archive/get/get.c:98`) and sends it per file in the local protocol payload (`src/command/archive/get/get.c:937`). With per-archive-id keys the pass must be sourced per archive-id when candidates are built (`src/command/archive/get/get.c:424`), since a single async batch can legitimately span an archive-id boundary during PITR -- the cache already tracks archive-ids per candidate and the payload is already per-file, so the change is contained. archive-push's per-repo cached pass (`src/command/archive/push/push.c:272`) stays correct as-is: a push batch writes only to the one current archive-id it loaded.
